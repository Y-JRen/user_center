    ...
    public function send($tpl_index, $phone, $code = null, $sms_service_type = 'luosimao', $params)
    {
        ...
        if ($sms_service_type == 'luosimao') { //luosimao 平台
            ...
            if (intval($tpl_index) >= 1000) {
                ...
                $str = ArrayHelper::getValue($this->luosimao_sms_tpl, $tpl_index);
               $num = preg_match_all('/{[a-z]+}/',$str,$matches);

                $flg = 1;
                foreach($matches[0] as $value){
                    if (in_array($value,$arrKeys)) {
                        continue;
                    }else{
                        $flg = 0;
                        break;
                ...
                if (count($arrKeys)<$num) {
                    return array('err_code'=>997,'msg'=>'参数数量有误');
                }elseif ($flg == 0) {
                    return array('err_code'=>997,'msg'=>'缺少短信模板对应的参数');
                }else{
                    $strContent = str_replace($arrKeys, array_values($params), $str);
                }

            } elseif (intval($tpl_index) == 1 || intval($tpl_index) == 2) {
                $strContent = sprintf($this->luosimao_sms_tpl[$tpl_index], $code);
            }
